...
bool DataBaseAccess::GetDeviceDetails(vector<int> &DID,
		...
		vector<int> &status, vector<string> &deviceName) {
	MYSQL_ROW row;
	MYSQL_RES *res = NULL;
	char query[SIZEOFCHARARRAYBIGGER];
	bool statusResult = false;
	...
	try {
		sprintf(query,
				"select DID,DEVICENAME,MACADDRESS,DEVICEIP,STATUS from deviceinfo");
		//	if (  ISDEBUG )  cout<<"query = "<<query<<endl;
		statusResult = !(mysql_query(mySqlConnection, query));
		if (!statusResult) {
			statusResult = StartQueryRecursion(query);
...
bool DataBaseAccess::GetDeviceInfo(char * deviceName, char * deviceip, int &DID,
	...
	try {
		sprintf(query,
				"select DID,MACADDRESS,DEVICENAME from deviceinfo where OWERES = 1 and DEVICEIP='%s'",
				deviceip);
		if (  ISDEBUG )  cout<<"query = " << query << endl;
		statusResult = !(mysql_query(mySqlConnection, query));
		if (!statusResult) {
...
int DataBaseAccess::InsertDeviceInfo(int DID, char* DeviceName, char *DeviceIP,
		int Status, int Owner, char* MacId) {
	char query[3000];
	bool statusResult;
	//int id =0;
	try {
		sprintf(query,
				"insert into `deviceinfo` (`DID`,`DEVICENAME`,`DEVICEIP`,`STATUS`,`OWERES`,`MACADDRESS`,`MACADDRESS`,`CREATETIMESTAMP`,`LASTMODIFIEDTIMESTAMP`) values ('%d','%s','%s',%d,%d,'%s',now(),now())",
				DID, DeviceName, DeviceIP, Status, Owner, MacId);
...
int DataBaseAccess::UpdateDeviceInfo(char* DID, char* DeviceName,
		char *DeviceIP, char* Status) {
	char query[3000];
	bool statusResult;
	int id = 0;
	try {
		sprintf(query,
				"update `deviceinfo` Set `DEVICENAME`='%s',`DEVICEIP`='%s',`STATUS`=%s,`LASTMODIFIEDTIMESTAMP`=now() where DID =%s ",
				DeviceName, DeviceIP, Status, DID);
...
void DataBaseAccess::GetAllDeviceInfo(vector<string> *DID,
		...
		vector<string> *Owner, vector<string> *MacAddress, bool withOwner ) {
	MYSQL_ROW row;
	MYSQL_RES *res = NULL;
	int rowCount = 0;
	char query[SIZEOFCHARARRAYBIGGER];
	...
	try {
		if ( withOwner )
			sprintf(query,"select DID,devicename,deviceip,status,oweres,macaddress from deviceinfo");
		else
			sprintf(query,"select DID,devicename,deviceip,status,oweres,macaddress from deviceinfo where oweres != 1");
...
bool DataBaseAccess::InsertOwnerDeviceIp(char * macAddress, int DID,
	...
	try {
		sprintf(query,
				"insert into deviceinfo values('%d','%downerNAS','%s','1','1','%s',NOW(),NOW())",
...
int DataBaseAccess::IsChannelsExist( int DID ){
	...
	bool statusResult = false;
	try {
		sprintf(query,"SELECT COUNT(*) FROM channelinfo where DID = '%d'",DID);
