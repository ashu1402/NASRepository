...
void XMLCreator::NewDeviceInfo(	const char* DID,const char* DName,const char* DeviceIP,const char* Status,const char* Owner,const char*  MacAddress, const char * backup)
{
	try{
		xml->AddElem("DID",DID);
...
void XMLCreator::NewFetchDeviceInfo(){
	try
	{
		vector<int> DID,status,backup;
		vector<string> macAddress,DeviceIP,deviceName;
		DataBaseAccessObj->GetDeviceDetails(DID,macAddress,DeviceIP,status,deviceName,backup);
		xml->OutOfElem();
		if ( !FindElement(xml->GetDoc(),"XMLDakshaImaging") )
			xml->AddElem( "XMLDakshaImaging" );
		xml->IntoElem();
...
void XMLCreator::NewFetchAllDevice()
{
	try
	{
		...
		DataBaseAccessObj->GetAllDeviceInfo(&DID,&DName,&DeviceIP,&Status,&Owner,&MacAddress,&backup ,true);

		for(int index=0;index < (int)DID.size(); index++ ){
			xml->AddElem("DeviceInfo");
			xml->IntoElem();
			NewDeviceInfo( DID[index].c_str(), DName[index].c_str(), DeviceIP[index].c_str(), Status[index].c_str() , Owner[index].c_str(),MacAddress[index].c_str(),backup[index].c_str() );
			xml->OutOfElem();
		}
		xml->OutOfElem();
	}catch (exception &e){
...
void XMLCreator::NewSuccessDeviceInfoAction( int DID,char *macAddress, char * deviceName, bool &backup){
	ReInitializeXML();
	xml->OutOfElem();
	if ( !FindElement(xml->GetDoc(),"XMLDakshaImaging") )
		xml->AddElem( "XMLDakshaImaging" );
