...
bool isCreateBackUpServer(){
	char choices[SIZEOFCHARARRAY];
	cout<<"\nDo you want to create this machine as backup server";
	restart:
	cout<<"\nEnter your choice(y/n)   ";
	gets(choices);
	if ( strcmp(choices,"y") == 0 )
		return true;
	else if ( strcmp(choices,"n") == 0 )
		return false;
	else{
		cout<<"\nInvalid choice\nRetry it";
		goto restart;
...
bool isAskAgainAboutActiveChannels(){
	char choices[SIZEOFCHARARRAY];
		cout<<"\nYou have choose backup server in this case active channel of same machine will be deactive";
		cout<<"\nDo you want to deactivate this machine channels";
		restart:
		cout<<"\nEnter your choice(y/n)   ";
		gets(choices);
		if ( strcmp(choices,"y") == 0 )
...
int main(int argc, char* argv[]){
	...
	bool isBackupServer = false;
	dBA->ClearRtspTable();
	int sysDID = dBA->GetSystemDID();
	if ( CheckValidation( dBA, serverIP ) ){
		if ( isCreateBackUpServer() ){
			cout<<"\nCreated Back up Server\n";
			if ( isAskAgainAboutActiveChannels() ){

				dBA->UpdateChannelInfoStatus(sysDID);
				dBA->UpdateDeviceBackupStatus(sysDID,1);
				isBackupServer = true;
			}
		}
		else{
			cout<<"\n Only Server\n";
			dBA->UpdateDeviceBackupStatus(sysDID,0);
		}
