...
bool XMLParser::ParseDeviceInfo(char *macValue, int &enteredDeviceID,char *xmlData, char * deviceName, bool &backUP){
		...
		if(nodeName=="Response"){
			...
			if(attributeName=="ActionName"){
				...
				if ( attributeValue == "GetDeviceInfo"){
					...
					xml_node<>* backup = getNodeByName("BACKUP",Success);

					if ( DID != NULL && macAddress != NULL && deviceName != NULL && backup != NULL ){
						enteredDeviceID = atoi(DID->value());
						sprintf( macValue ,"%s",macAddress->value());
						sprintf( deviceName ,"%s",deviceNameInfo->value());
						backUP = atoi(backup->value());
						cout<<'\nbackUP'<<backUP<<endl;
...
bool XMLParser::GetDeviceInfo( int &enteredDeviceID,char *macValue, char * DeviceIP, int sockFD, char * deviceName, bool & backup ){
	...
	if(recvData(&size,sizeof(int),sockFD)){
		if ( recvData(xmlData,size, sockFD) ){
			...
			return ParseDeviceInfo(macValue,enteredDeviceID,xmlData,deviceName, backup);
		}
	}
	return false;
}
		...
		if ( !DataBaseAccessObj->IsServerExist( DeviceIP->value(), NULL,enteredDeviceID,tempIP) ){
			if ( LookingForServer((DeviceIP->value()),55555,sockFD) ){
				if( DeviceIP == NULL ){
					...
					GetDeviceInfo ( enteredDeviceID,macValue, DeviceIP->value(),sockFD,deviceName, backValue );

					int ret = DataBaseAccessObj->InsertDeviceInfo ( enteredDeviceID, deviceName,DeviceIP->value(),statusValue,ownerValue,macValue,backValue);
					if(ret){
	...
	try{
		...
		if(DeviceInfo==NULL ){
			...
			for (;DeviceInfo && (DeviceInfo != NULL);DeviceInfo=DeviceInfo->next_sibling("DeviceInfo") ){
				...
				xml_node<>* backup = getNodeByName("BACKUP",DeviceInfo);

				if ( DID != NULL && DeviceIp != NULL && DeviceName != NULL && macAddress != NULL && Status != NULL && backup != NULL ){
					bool backValue = false;
					if ( atoi(backup->value()) == 1 )
						backValue = true;
					if ( atoi(DID->value()) != sysDID ){
						...
						if ( ! ret ){
							...
							result = DataBaseAccessObj->InsertDeviceInfo(atoi(DID->value()),DeviceName->value(),DeviceIp->value(),atoi(Status->value()),0,macAddress->value(),backValue);
							...
							cout<<"\nDID"<<DID->value()<<endl;
							cout<<"\nDID"<<DID->value()<<endl;
							result = DataBaseAccessObj->UpdateDeviceInfo(DID->value(),DeviceName->value(),DeviceIp->value(),Status->value(), backup->value());
						}
						if ( result )
...
bool XMLParser::DeviceInfo(xml_node<>* node){
	try{
		...
		if(0){
			...
			xml_node<>* DeviceIP = getNodeByName("DeviceIP",node);
			bool backup = false;
			if ( DataBaseAccessObj->GetDeviceInfo(deviceName, DeviceIP->value(),DID, macAddress, backup ) )
				XMLCreatorObj->NewSuccessDeviceInfoAction(DID,macAddress,deviceName,backup);
			else
